...
**换棚（改苗床归属的温室）必须和床位账同一笔核对**，不是只改温室卡片上的归属字：

- 先和开批次/转棚一样锁住这张床行，再锁定读床位账；床上还有没出圃批次的在占段
  （开放段，含已经排上、还没到播种日的未来档期；或覆盖今天的段，含未来起迁、今天苗还没迁走的旧床段）时，
  **整笔拒绝换棚**，报出占着床的是哪批苗；只有已经空出来的床（在占段都在过去、批次已出圃/报废）才能挪。
- 「换棚」和「往这张床开新批次」抢同一把床行锁，两笔只有一笔成功：
  先到换了棚，后到的开批次锁后重读会发现床的归属棚/状态变了，报「苗床已经换到 X 棚」或「现在是维修」；
  先到开了批次，后到的换棚会在床位账上读到新占用段而失败，报「已被批次 NB-xxxx 占着」。
- 维修中的床同样串行在这把锁上：不会一边被换棚、一边被当成能开新批次。
- 任何一步不通过都抛错回滚，不会出现「床已经挂到新棚、床位账日期段还写着旧棚」的半截账。
